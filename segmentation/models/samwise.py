...
class SAMWISE(nn.Module):
    def __init__(self,
        ...
        for i in range(len(fusion_stages)):
            self.cmt_adapters.append(CMT_adapter(
                                        ...
                                        view_token_dim=text_encoder_embed_dim,
        ...
        self.view_embeddings = nn.Embedding(3, text_encoder_embed_dim)
        self.view_prompt_mlp = MLP(input_dim=text_encoder_embed_dim, hidden_dim=text_encoder_embed_dim, output_dim=self.sam.sam_prompt_embed_dim, num_layers=1)

        self.memory_bank = {} # to store all frames memory
        self.last_frame_cme_applied = 0
    ...
    def get_extra_state(self):
        ...
        return {
            "memory_bank": self._serialize_memory_bank(self.memory_bank),
            "last_frame_cme_applied": int(self.last_frame_cme_applied),
        }
    ...
    def set_extra_state(self, state):
        ...
        self.memory_bank = self._deserialize_memory_bank(memory_bank, device)
        self.last_frame_cme_applied = int(state.get("last_frame_cme_applied", 0))
    ...
    def compute_backbone_output(self, samples, captions, targets):
        ...
        B, T = BT
        view_tokens_b = self._build_view_embedding_tokens(targets, B, T, samples.device)
        ...
        view_tokens_bt = self._scale_view_prompt(view_tokens_bt)
    ...
    def _early_fusion_stage(self, T, samples, txt, attention_mask, view_tokens_b):
        ...
        for i, (i_v, i_t) in enumerate(zip(fusion_vis[:-1], fusion_txt[:-1])):
            ...
            if i in self.fusion_stages:
                v = vis.clone()
                t = txt.clone()
                view = view_tokens_b.clone()
                # v, t = self.cmt_adapters[self.fusion_stages.index(i)](v.permute(0, 3, 1, 2), T, t)
                v, t, view = self.cmt_adapters[self.fusion_stages.index(i)](v.permute(0, 3, 1, 2), T, t, view_tokens=view)
                vis = vis + v.permute(0, 2, 3, 1)
                txt = txt + t
                view_tokens_b = view_tokens_b + view
